...
// Lightweight version stamp to surface in health checks / diagnostics
const proxyVersion = '2025-11-09';
...
app.post('/api/jira/issues/search', async (req, res) => {
  try {
    const { domain, email, apiToken, projectIdOrKey, statusCategory, startAt = 0, maxResults = 50, excludeDescription = false, pageToken = null, fields, jql: rawJql, expand } = req.body || {};
    if (!domain || !email || !apiToken) {
      return res.status(400).json({ error: 'Missing domain, email, or apiToken' });
    }
    // Allow a direct JQL search without requiring projectIdOrKey (advanced usage)
    if (!projectIdOrKey && !rawJql) {
      return res.status(400).json({ error: 'Provide projectIdOrKey or an explicit jql string.' });
    }
    ...
    let jql;
    if (typeof rawJql === 'string' && rawJql.trim()) {
      jql = rawJql.trim();
    } else {
      // Build a safer project clause: numeric IDs unquoted, simple uppercase keys unquoted, else quoted
      function buildProjectClause(val) {
        if (!val) return null;
        const v = String(val).trim();
        if (/^\d+$/.test(v)) return `project = ${v}`; // numeric ID
        if (/^[A-Z][A-Z0-9_]*$/.test(v)) return `project = ${v}`; // typical key pattern
        return `project = "${v.replace(/"/g, '\\"')}"`; // quote anything else
      }
      const jqlParts = [buildProjectClause(projectIdOrKey)];
      if (statusCategory && typeof statusCategory === 'string') {
        const sc = String(statusCategory).trim();
        if (sc) jqlParts.push(`statusCategory = "${sc.replace(/"/g, '\\"')}"`);
      }
      jql = jqlParts.filter(Boolean).join(' AND ') + ' ORDER BY updated DESC';
    }
    ...
    // If caller supplies explicit fields array, honor it (sanitized to strings)
    if (Array.isArray(fields) && fields.length > 0) {
      baseFields = fields.filter(f => typeof f === 'string' && f.trim()).map(f => f.trim());
    }
    ...
    if (typeof expand === 'string' && expand.trim()) {
      body.expand = expand.trim(); // e.g. 'names,schema'
    }
    ...
    // If the new endpoint rejects with 400 invalid payload, retry using classic /search
    if (!resp.ok && resp.status === 400) {
      const errText = (text || '').toLowerCase();
      const looksLikePayloadIssue = errText.includes('invalid request payload');
      if (looksLikePayloadIssue) {
        ({ resp, text } = await doSearch('/search'));
        fallbackUsed = true;
        fallbackReason = 'jql400-invalid-payload';
      }
    }

    // Also optionally fallback on 404 if explicitly enabled via env
    if (!resp.ok && resp.status === 404 && legacyFallbackEnabled) {
      ({ resp, text } = await doSearch('/search'));
      fallbackUsed = true;
      fallbackReason = fallbackReason || 'jql404-not-found-env';
    }
...
// Curated recommended field set for issue searches.
// This gives the UI a quick-start list without calling /field first.
// Caller may POST { excludeDescription: true } to omit description for performance.
app.post('/api/jira/issues/recommendedFields', (req, res) => {
  try {
    const { excludeDescription = false } = req.body || {};
    // Base set chosen to balance usefulness vs payload size.
    const base = [
      'summary',
      'status', // always useful for boards
      'assignee',
      'labels',
      'components',
      'fixVersions',
    ...
    if (!excludeDescription) base.splice(1, 0, 'description'); // insert after summary
    res.json({
      fields: base,
      excludeDescription: !!excludeDescription,
      usage: 'Supply this array as `fields` in /api/jira/issues/search. Extend by pushing customfield_XXXXX IDs discovered via /api/jira/fields.',
...
app.get('/api/health', (req, res) => {
  res.json({
    ...
    routes: [
      ...
      '/api/jira/issues/recommendedFields',
